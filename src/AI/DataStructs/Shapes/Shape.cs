...
public class Shape : IEquatable<Shape>
{
    ...
    public bool FuzzyEquals(Shape other)
    {
        ...
        // Сравнение «значимых» измерений — игнорируем все единичные (singleton) оси.
        // Это позволяет корректно сопоставлять формы разного ранга, представляющие одни и те же данные,
        // например Shape1D(N) = [N] и Shape3D(H=N, W=1, D=1) = [1, N, 1].
        int[] a = SqueezeOnes(_values);
        int[] b = SqueezeOnes(other._values);
    ...
    private static int[] SqueezeOnes(int[] values)
    {
        int cnt = 0;
