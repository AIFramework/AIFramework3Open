        ...
        RunState run, [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        while (!stopped && step < _config.MaxIterations)
        {
            ...
            if (exhausted != null)
            {
                ...
                // набора, цикл продолжается остальными. Раньше два отказа подряд гасили весь прогон,
                // и собранное до них уходило в ответ по накопленному. Системная инструкция при этом
                // НЕ перестраивается (дисциплина кэша: префикс не мутирует посреди прогона); о
                // блокировке модель узнает подсказкой к сообщению шага, а перечень схемы решения
                // вызывающий строит по набору шага.
