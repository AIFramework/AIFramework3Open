...
public static class DataMethods
{
    ...
    public static SKColor GetColorForStyle(string path)
    {
        using (SKBitmap decoded = SKBitmap.Decode(path))
        {
            ...
            using (SKBitmap small = decoded.Resize(new SKImageInfo(20, 20), SKFilterQuality.Medium))
            {
                double r = 0, g = 0, b = 0;
                for (int i = 0; i < 20; i++)
                {
                ...
                return new SKColor((byte)(r / 400.0), (byte)(g / 400.0), (byte)(b / 400.0));
            }
        }
    }
