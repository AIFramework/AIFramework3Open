...
/// <summary>
/// Алгоритм D* Lite — инкрементальный поиск кратчайшего пути на сетке,
/// поддерживающий динамические изменения (блокировка/разблокировка ячеек).
/// Планирование ведётся от цели к старту.
/// </summary>
[Serializable]
public class DStarLite
{
    ...
    /// <summary>
    /// Создаёт экземпляр D* Lite для сетки заданного размера
    /// </summary>
    ...
    public DStarLite(int width, int height, (int X, int Y) start, (int X, int Y) goal)
    {
        _width = width;
        _height = height;
        _start = start;
        ...
        _rhs[_goal.X, _goal.Y] = 0;
        var key = CalculateKey(_goal.X, _goal.Y);
        _openSet.Add((key, _goal.X, _goal.Y));
        _openKeys[(_goal.X, _goal.Y)] = key;
    ...
    private (double K1, double K2) CalculateKey(int x, int y)
    {
        ...
        double h = Heuristic(x, y, _start.X, _start.Y);
        return (minGRhs + h + _km, minGRhs);
    }
    ...
    {
        var startKey = CalculateKey(_start.X, _start.Y);
        ...
        while (_openSet.Count > 0)
        {
            ...
            var curStartKey = CalculateKey(_start.X, _start.Y);
            bool startInconsistent = Math.Abs(_rhs[_start.X, _start.Y] - _g[_start.X, _start.Y]) > 1e-9;

            if (CompareKeys(topKey, curStartKey) >= 0 && !startInconsistent)
                break;
            ...
            var newKey = CalculateKey(ux, uy);

            if (CompareKeys(topKey, newKey) < 0)
            {
                _openSet.Add((newKey, ux, uy));
                _openKeys[(ux, uy)] = newKey;
            }
            else if (_g[ux, uy] > _rhs[ux, uy])
            {
                _g[ux, uy] = _rhs[ux, uy];
                foreach (var (dx, dy) in Dirs)
                {
    ...
    /// <summary>
    /// Устанавливает или снимает блокировку ячейки сетки
    /// </summary>
    ...
    /// <summary>
    /// Перепланирует маршрут с учётом изменений на сетке
    /// </summary>
    public void Replan()
    {
    ...
    /// <summary>
    /// Возвращает текущий кратчайший путь от старта до цели
    /// </summary>
    ...
    public List<(int X, int Y)> GetPath()
    {
        ...
        if (_g[_start.X, _start.Y] >= double.MaxValue)
            return path;

        int cx = _start.X, cy = _start.Y;
        path.Add((cx, cy));

        int maxSteps = _width * _height;
        int steps = 0;

        while ((cx != _goal.X || cy != _goal.Y) && steps++ < maxSteps)
        {
