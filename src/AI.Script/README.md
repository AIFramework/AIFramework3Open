...
Хранилище не обязано быть папкой. Модули читают и пишут байты через `IScriptSandbox` по
относительному пути, поэтому файлы можно держать в памяти (`MemorySandbox`) или отдать своими
силами — из объектного хранилища, из вложений чата. Правило «наружу нельзя» живёт в общей
нормализации пути (`ScriptPaths.Normalize`), и новое хранилище получает его даром:

```csharp
var files = new MemorySandbox(readOnly: true).Put("sales.csv", csvBytes);
RunResult result = await host.RunAsync("emit n = len(io.load(\"sales.csv\"))", new RunOptions { Sandbox = files });
```
...
```csharp
[ScriptModule("myco", "Функции нашего проекта", Group = "данные")]
...
Ещё три параметра атрибутов заменяют отдельную регистрацию:

- `Group` у модуля — задача, под которой он стоит в индексе промпта (список задач закрыт:
  `ManifestGroups.All`);
- `Reads = "xlsx"` у функции — расширения, которые она открывает по пути первым аргументом;
  `io.load` выбирает по ним читателя, ничего не зная о модуле;
- `Columns = "path,size"` у функции, возвращающей таблицу постоянного вида, — по ним проверка
  до запуска ловит опечатки в именах колонок ниже по конвейеру.
