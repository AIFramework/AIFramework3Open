...
public static class FastConv
{
    ...
    /// <summary>
    /// Быстрая корреляция (перегрузка для Vector)
    /// </summary>
    /// <param name="signal">Вектор сигнала</param>
    public static Vector FastCorrelation(Vector signal)
        => new Vector(FastCorrelation(signal.ToArray()));
