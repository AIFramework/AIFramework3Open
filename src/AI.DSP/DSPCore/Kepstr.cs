...
public static class Cepstrum
{
	...
	public static Vector FKT(Vector signal)
    {
        ...
        Vector spectrPow = 2 * spectr.MagnitudeVector.Transform(x => Math.Log(Math.Max(x, 1e-30)));
	...
	public static Vector WelchFKT(Vector signal, int window_size_1)
    {
        ...
        Vector spectrPow = Welch.WelchRun(signal, window_size_1, 0.5, window_1).Transform(x => Math.Log(Math.Max(x, 1e-30)));
        return FFT.CalcFFT(spectrPow).RealVector / spectrPow.Count;
    }
