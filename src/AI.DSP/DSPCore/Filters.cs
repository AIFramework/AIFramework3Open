...
public static class Filters
{
    ...
    public static Vector FilterKontur(Vector st, double Q, double f0, int fd)
    {
        ...
        for (int i = 1; i < f.Count / 2; i++)
            kw[f.Count - i] = Complex.Conjugate(kw[i]);
    ...
    /// </summary>
    /// <param name="f">Вектор частот</param>
    /// <param name="param">Параметры в формате "тип:частота1[:частота2]"</param>
    ...
    public static Vector CreatAFH(Vector f, string[] param)
    {
        ...
        {
            string[] parts = param[i].Split(':');
            if (parts.Length < 2)
                throw new FormatException($"Invalid filter parameter format at index {i}: '{param[i]}'. Expected 'type:freq1[:freq2]'.");
            ...
            switch (parts[0])
            {
                ...
                case "b":
                    if (parts.Length < 3)
                        throw new FormatException($"Filter type '{parts[0]}' requires two frequency parameters at index {i}: '{param[i]}'.");
                    fP = new double[] { Convert.ToDouble(parts[1]), Convert.ToDouble(parts[2]) };
                    kw *= GetAFH(f, fP, parts[0] == "r" ? AFHType.Rezector : AFHType.Band).CutAndZero(kw.Count);
                    break;
                case "l":
                    fP = new double[] { Convert.ToDouble(parts[1]) };
                ...
                case "h":
                    ...
                    break;
                default:
                    throw new FormatException($"Unknown filter type '{parts[0]}' at index {i}.");
            }
        }
