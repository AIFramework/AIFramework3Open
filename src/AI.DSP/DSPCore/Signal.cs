...
public static class Signal
{
    ...
    {
        double df = fd / Count;
        return FunctionsForEachElements.GenerateTheSequence(-Count / 2.0 * df, df, (Count / 2.0 - 1) * df).CutAndZero(Count);
    }
    ...
    /// <returns></returns>
    public static double Energy(Vector signal, double fd)
    {
        double energy = Functions.Summ(signal.Transform(x => Math.Pow(x, 2)));
        return energy / fd;
    }
    ...
    /// <param name="newfd">Новая частота дискретизации</param>
    /// <returns>Вектор той же длительности, но с более высокой частотой дискретизации</returns>
    public static Vector Resample(Vector inp, int fd, int newfd)
    {
        if (fd <= 0) throw new ArgumentException("fd must be positive", nameof(fd));
        if (newfd <= 0) throw new ArgumentException("newfd must be positive", nameof(newfd));
        if (newfd % fd != 0) throw new ArgumentException("newfd must be an integer multiple of fd");
        if (inp == null || inp.Count == 0) throw new ArgumentException("Input vector must not be empty", nameof(inp));

        int k = newfd / fd;
        ComplexVector inputSpectr = FFT.CalcFFT(inp);
        int lenFull = inp.Count * k;
        int halfN = inp.Count / 2;
        ComplexVector cV = new ComplexVector(lenFull);

        for (int i = 0; i < halfN; i++)
            cV[i] = inputSpectr[i];

        for (int i = halfN; i < lenFull - halfN; i++)
            cV[i] = new System.Numerics.Complex(0, 0);

        for (int i = 0; i < halfN; i++)
            cV[lenFull - halfN + i] = inputSpectr[halfN + i];
