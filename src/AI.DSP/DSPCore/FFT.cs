...
public class FFT
{
    ...
    private readonly int HalfSamplesCount;
    ...
    public FFT(int len)
    {
        SamplesCount = Functions.NextPow2(len);
        HalfSamplesCount = SamplesCount / 2;
        rotateCoefficiens = new Complex[SamplesCount]; // [0 : HalfSample-1] — exp(-jwt), [HalfSample : SamplesCount-1] — exp(jwt) 

        CalcRotate(SamplesCount, HalfSamplesCount, rotateCoefficiens);
    }
    ...
    private static unsafe void CalcRotate(int samplesCount, int halfSamplesCount, Complex[] rotateCoefficiens)
    {
        fixed (Complex* rotateCoef = rotateCoefficiens)
        {
            Complex* pointerToRotateCoef = rotateCoef;

            for (int i = 0; i < halfSamplesCount; i++)
                *pointerToRotateCoef++ = Complex.Exp(-1 * 2 * Complex.ImaginaryOne * Math.PI * i / samplesCount);

            for (int i = 0; i < halfSamplesCount; i++)
                *pointerToRotateCoef++ = Complex.Exp(2 * Complex.ImaginaryOne * Math.PI * i / samplesCount);
        }

    }

    // Поворотные множ. для одного типа преобразования
    private static unsafe void CalcRotate(int samplesCount, int halfSamplesCount, Complex[] rotateCoefficiens, bool canonic)
    {
        fixed (Complex* rotateCoef = rotateCoefficiens)
        {
            Complex* pointerToRotateCoef = rotateCoef;
    ...
    {
        return SimpleFFT(input, SamplesCount, HalfSamplesCount, rotateCoefficiens, canonic);
    }

    // Статический вызов
    private static ComplexVector FftS(ComplexVector input, bool canonic)
    {
        return AIDSPSettings.FFTCore(input, canonic);
    }
    ...
    {
        ComplexVector compInp = new ComplexVector(SamplesCount);
    ...
    /// <summary>
    /// Расчет БПФ (перегрузка для Vector)
    /// </summary>
    /// <param name="inp">Входной вектор</param>
    /// <param name="minCount">Минимальный размер (степень двойки), -1 = авто</param>
    /// <param name="canonic">Каноническое представление</param>
    public static Complex[] CalcFFT(Vector inp, int minCount = -1, bool canonic = true)
        => CalcFFT(inp.ToArray(), minCount, canonic);

    /// <summary>
    /// Расчет обратного БПФ (перегрузка для Vector)
    /// </summary>
    /// <param name="inp">Входной массив комплексных чисел</param>
    /// <param name="size">Размер выходного вектора (-1 = авто)</param>
    public static Vector CalcIFFTRealVector(Complex[] inp, int size = -1)
        => new Vector(CalcIFFTReal(inp, size));
    ...
    public Vector GetSpectrum(Vector input, Func<int, Vector> window)
    {
        ...
        vect = this.CalcFFT(vect).MagnitudeVector / (SamplesCount / 2.0); // Амплитуды
        return vect.CutAndZero(SamplesCount / 2); // Половина вектора
    }
    ...
    {
        return BaseFFT(inp, false) / SamplesCount;
    }
