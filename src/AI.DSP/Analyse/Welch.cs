...
public class Welch
{
    ...
    {
        if (signal.Count < window)
            throw new ArgumentException("Signal length must be at least equal to window size", nameof(signal));

        FFT fft = new FFT(window);
        int step = Math.Max((int)((1.0 - overlap) * window), 1);
        ...
        {
            n++;
            Vector data = signal.GetInterval(i, i + window);
            data *= windowForFFT;
        ...
        if (n == 0)
            throw new InvalidOperationException("No segments were processed. Check signal length and window size.");
