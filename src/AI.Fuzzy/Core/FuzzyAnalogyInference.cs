...
public class FuzzyAnalogyInference
{
    ...
    public static Matrix GetImplicationMatrixG(Vector @if, Vector then)
    {
        ...
        for (int i = 0; i < @if.Count; i++)
        {
            ...
            {
                ef[i, j] = FLV.GImplication(@if[i], then[j]);
            }
        }
    ...
    /// <summary>
    /// Матрица импликации Мамдани (минимум) между дискретными нечёткими векторами.
    /// Агрегирование и дефаззификация: класс FuzzyMamdaniInference (пространство имён AI.Fuzzy.Inference).
    /// </summary>
    public static Matrix GetImplicationMatrixMamdani(Vector ifVector, Vector thenVector)
    {
        return global::AI.Fuzzy.Inference.FuzzyMamdaniInference.GetImplicationMatrixMamdani(ifVector, thenVector);
    }

    /// <summary>
    /// Матрица импликации Ларсена (произведение). Агрегирование и дефаззификация: класс FuzzyLarsenInference (пространство имён AI.Fuzzy.Inference).
    /// </summary>
    public static Matrix GetImplicationMatrixLarsen(Vector ifVector, Vector thenVector)
    {
        return global::AI.Fuzzy.Inference.FuzzyLarsenInference.GetImplicationMatrixLarsen(ifVector, thenVector);
    }
