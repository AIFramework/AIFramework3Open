...
public static class Cointegration
{
    ...
    {
        (double[] values, double[,] vectors) = LinearAlgebra.SymmetricEigen(matrix);
        int n = values.Length;

        var scaled = new double[n, n];
        for (int i = 0; i < n; i++)
            for (int j = 0; j < n; j++)
                scaled[i, j] = vectors[i, j] / Math.Sqrt(Math.Max(values[j], 1e-12));

        return LinearAlgebra.Multiply(scaled, LinearAlgebra.Transpose(vectors));
    }
}
