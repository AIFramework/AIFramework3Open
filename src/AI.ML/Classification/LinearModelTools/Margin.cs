...
public class Margin
{
    ...
    /// <returns></returns>
    public static VectorDatasetItem[] GetSupportVector(Vector ideal, Vector[] inp, Vector w, double b, int supportVectorsCount = 1)
    {
        int count = Math.Min(supportVectorsCount, inp.Length);
        VectorDatasetItem[] vectorClasses = new VectorDatasetItem[inp.Length];
        VectorDatasetItem[] data = new VectorDatasetItem[count];

        _ = Parallel.For(0, inp.Length, i =>
        {
            VectorDatasetItem vectorClass = new VectorDatasetItem()
            {
        ...
        System.Collections.Generic.List<VectorDatasetItem> vectorClassesList = vectorClasses.ToList();
