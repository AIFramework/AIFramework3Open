...
public class NN : IClassifier
{
		...
		public void AddClass(Vector[] tDataset, int numClass)
    {
        ...
        _classes.Add(new VectorDatasetItem(a, numClass));
    }
    /// <summary>
    ...
    /// </summary>
    /// <param name="dataset">Набор данных признаки-метка</param>
    public void TrainGroup(VectorDataset dataset)
    {
        VectorDataset data = dataset.GroupMean();
    ...
    /// <param name="path">Путь до файла</param>
    /// <returns></returns>
    public static NN Load(string path)   => SafeSerializer.Load<NN>(path, AiMlJsonOptions.Default);
    ...
    /// <param name="stream">Поток</param>
    /// <returns></returns>
    public static NN Load(Stream stream) => SafeSerializer.Load<NN>(stream, AiMlJsonOptions.Default);
}
