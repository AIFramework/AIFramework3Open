...
public class MCFast
{
    ...
    public void Save(string path)   => AI.DataStructs.SafeSerializer.Save(path, this, AI.ML.Json.AiMlJsonOptions.MCFastOptions);

    /// <summary>
    /// Сохранение Марковской модели в поток
    /// </summary>
    /// <param name="stream">Поток</param>
    public void Save(Stream stream) => AI.DataStructs.SafeSerializer.Save(stream, this, AI.ML.Json.AiMlJsonOptions.MCFastOptions);
    #endregion
    ...
    /// <param name="path">Путь до файла</param>
    /// <returns></returns>
    public static MCFast Load(string path)   => AI.DataStructs.SafeSerializer.Load<MCFast>(path, AI.ML.Json.AiMlJsonOptions.MCFastOptions);
    ...
    /// <param name="stream">Поток</param>
    /// <returns></returns>
    public static MCFast Load(Stream stream) => AI.DataStructs.SafeSerializer.Load<MCFast>(stream, AI.ML.Json.AiMlJsonOptions.MCFastOptions);
    #endregion

    #endregion

    // Bridge-доступ для MCFastJsonConverter (тот же сборка)
    internal Dictionary<int[], Dictionary<int, double>> InternalData => _dataMC;
    internal void ReconstructAfterLoad() => CreateVector();
