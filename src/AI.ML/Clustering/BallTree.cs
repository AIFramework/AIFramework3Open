...
public class BallTree
{
    ...
    private BallTreeNode BuildTree(Vector[] dataset, int depth)
    {
        if (dataset.Length == 1)
        {
            return new BallTreeNode(dataset[0]) { Depth = depth };
        }
        ...
        // Создаем узел дерева с выставленной глубиной
        var node = new BallTreeNode
        {
            Point = sortedDataset[medianIndex],
            Depth = depth,
            Left = left.Length > 0 ? BuildTree(left, depth + 1) : null,
            Right = right.Length > 0 ? BuildTree(right, depth + 1) : null
        };
