...
public class FastKMeans : IClustering
{
    ...
    public void Train(Vector[] dataset, int seed = 10)
    {
        ...
        while (isChangedLabel && count < MaxCount)
        {
            ...
            var labeledVectors = dataset.Select((vector) => new VectorDatasetItem { Features = vector, ClassMark = NearestVector(vector, ballTree) }).ToArray();

            // Обновление центроидов
            int dim = dataset[0].Count;
            Vector[] newCentroids = new Vector[GroupCount];
            int[] labeledCount = new int[GroupCount];

            for (int i = 0; i < GroupCount; i++)
                newCentroids[i] = new Vector(dim);
    ...
    /// <summary>Ближайший вектор (евклидова метрика)</summary>
