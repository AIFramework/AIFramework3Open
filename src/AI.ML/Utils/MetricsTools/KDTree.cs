...
namespace AI.ML.Utils.MetricsTools;
...
public class KDTree
{
    ...
    public KDNode NearestNeighbor(double[] point, int depth = 0, KDNode best = null)
    {
        ...
        // Проверяем «другую» ветку (противоположную той, в которую пошли)
    KDNode otherBranch = (point[axis] < Root.DataVector[axis]) ? Root.Right : Root.Left;
    if (otherBranch != null && Math.Abs(point[axis] - Root.DataVector[axis]) < Dist(point, nextBest.DataVector))
    {
        nextBest = NearestNeighborRecursive(point, otherBranch, depth + 1, nextBest);
    }
    ...
    private KDNode NearestNeighborRecursive(double[] point, KDNode node, int depth, KDNode best)
    {
        ...
        // Проверяем «другую» ветку (противоположную той, в которую пошли)
        KDNode otherBranch = (point[axis] < node.DataVector[axis]) ? node.Right : node.Left;
        if (otherBranch != null && Math.Abs(point[axis] - node.DataVector[axis]) < Dist(point, nextBest.DataVector))
        {
            nextBest = NearestNeighborRecursive(point, otherBranch, depth + 1, nextBest);
        }
