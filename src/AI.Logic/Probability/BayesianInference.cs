...
public class BayesianInference
{
    ...
    public void UpdateProbabilities(Dictionary<string, double> likelihoods, Dictionary<string, double> hypotheses)
    {
        ...
        if (evidenceProbability < 1e-300)
        {
            // Нулевая нормировочная константа — равномерное апостериорное распределение
            double uniform = hypotheses.Count > 0 ? 1.0 / hypotheses.Count : 0.0;
            foreach (var key in hypotheses.Keys)
        ...
        foreach (var hypothesis in hypotheses)
        {
            ...
            // Для гипотез без данных о правдоподобии оставляем априорную вероятность
            else if (!probabilities.ContainsKey(hypothesis.Key))
            {
                probabilities[hypothesis.Key] = hypothesis.Value;
            }
        }
    }
