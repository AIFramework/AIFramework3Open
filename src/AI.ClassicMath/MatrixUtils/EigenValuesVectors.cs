...
public class EigenValuesVectors
{
    ...
    private static Vector GetEigenvector(Matrix a, double eigenvalue)
    {
        ...
        // Tikhonov: (A - λI) бывает численно вырождена (погрешность QR, близкие λ).
        // Сдвиг по диагонали сохраняет направление v после нормировки лучше, чем падение Гаусса.
        double sumAbs = 0;
        int n = a.Width * a.Height;
        for (int r = 0; r < a.Height; r++)
            for (int c = 0; c < a.Width; c++)
                sumAbs += Math.Abs(a[r, c]);
        double avgAbs = n > 0 ? sumAbs / n : 1.0;
        double ridge = Math.Max(1e-14, 1e-10 * avgAbs);
        for (int i = 0; i < a.Width; i++)
            coef[i, i] += ridge;
