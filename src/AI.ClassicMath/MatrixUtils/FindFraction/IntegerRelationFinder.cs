...
/// <summary>
/// Алгоритм поиска целочисленных полиномиальных коэффициентов
...
public static class IntegerRelationFinder
{
    ...
    private static double EvaluatePolynomialAbs(int[] coeffs, double x)
    {
        double result = 0;
        double xPow = 1;
        for (int i = 0; i < coeffs.Length; i++)
        ...
        for (int c = -50; c <= 50; c++)
        {
            for (int e = -50; e <= 50; e++)
            {
                ...
                if (Math.Abs(error) < tolerance)
                {
                    ...
                    double checkError = EvaluatePolynomialAbs(coeffs, x);
