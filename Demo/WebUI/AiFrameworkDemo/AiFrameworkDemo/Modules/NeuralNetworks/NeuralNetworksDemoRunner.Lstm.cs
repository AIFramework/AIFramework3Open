...
{
    #region LSTM sequence prediction

    private static void RunLstmSequenceCase(
        ...
        ChartView cv, ref string? textOut)
    {
        ...
        var model = new LstmPredictor(inputSize: 1, hiddenSize: H, rng: rng);
        ...
        cv.ChartName = $"LSTM({H})  —  окно {window}, горизонт {predLen}, MSE={mse:F4}";
        ...
        textOut = $"LSTM({H})  window={window}  epochs={epochs}  lr={lr}  predLen={predLen}  freq={freq:F3}  MSE={mse:F5}";
    }

    private sealed class LstmPredictor : Module
    {
        private readonly LSTM   _lstm;
        private readonly Linear _head;

        public LstmPredictor(int inputSize, int hiddenSize, Random rng)
        {
            _lstm = RegisterModule("lstm", new LSTM(inputSize, hiddenSize, true, true, rng));
        ...
        {
            int T = input.Shape[1];
            var (outputs, _, _) = _lstm.ForwardSeq(input);
