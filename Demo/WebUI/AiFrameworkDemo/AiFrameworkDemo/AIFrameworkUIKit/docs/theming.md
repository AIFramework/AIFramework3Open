...
| `--aif-cur-a08/a12/a20/a30` | Лестница прозрачностей контекстного цвета |
| `--aif-cur-scroll`       | Скроллбар внутри контекста        |

Лестница `--aif-cur-a*` появилась потому, что компоненты задавали оттенки
акцента литералами `rgba(99, 102, 241, α)`. Такой литерал не реагирует на
`[data-color]`: карточка теории оставалась индиговой даже в изумрудном
модуле. Пишите так:

```css
/* было: индиго навсегда */
.aif-theory-subtitle { background: rgba(99, 102, 241, 0.1); }

/* стало: следует цвету модуля, с запасным значением вне контекста */
.aif-theory-subtitle { background: var(--aif-cur-a12, var(--aif-accent-a12)); }
```

Второй аргумент `var()` обязателен: вне `[data-color]` контекстные токены
не определены, и без запасного значения свойство просто не применится.
