...
Сборка **`AI.Fuzzy`** (`AI.Fuzzy.dll`, **.NET Standard 2.0**) содержит типы для нечётких множеств, импликаций, типовых методов нечёткого вывода (Мамдани, Ларсен, Сугено, Цукамото), нечёткого регулятора в духе PID, а также гибридные ML-компоненты (`FuzzyClassifier`, `LingVarGaussian`). **Зависимости по `ProjectReference`:** **`AI`** (векторы и матрицы из `AI.DataStructs.Algebraic`), **`AI.ML`** (базовые классификаторы и т.д.), **`AI.KNN`** (для `LingVarGaussian` и связанных сценариев).
