...
Сборка **`AI.ControlSystems`** (`AI.ControlSystems.dll`, **.NET Standard 2.0**) содержит типы для учебных сценариев **моделирования объекта управления** и **обратной модели** (по состоянию — восстановить управление) на базе регрессии `AI.ML` (в т.ч. `NeuralMultyRegression`), а также **классические дискретные PID-регуляторы**. Зависимости: **`AI`**, **`AI.ML`**.
