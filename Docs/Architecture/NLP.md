...
### Что возвращает фасад

`Lemmatizer.CreateRussian()` — тот вход, которым пользуется остальной код (в том числе
модуль `nlp.lemma` в языке сценариев), — возвращает морфологический разбор. Прежнее
поведение, одни суффиксальные правила без определения части речи, доступно под именем
`Lemmatizer.CreateRussianRules()`: оно разбирает хуже, но почти никогда не портит слово.

Выбор по умолчанию сделан в пользу качества, и цена у него та же, что в таблице выше:
ошибка теперь чаще приводит слово к основе, чем оставляет нетронутым.

### Из языка сценариев

Морфология доступна в пространстве `nlp` языка сценариев — то есть и скрипту,
и языковой модели через манифест возможностей:

| Функция | Что делает |
|---------|------------|
| `nlp.lemma(w)` | Лемма слова или списка слов |
| `nlp.pos(w)` | Часть речи кодом: `NOUN`, `VERB`, `ADJ`, … |
| `nlp.analyze(w)` | Запись: слово, лемма, часть речи и её русское название |
| `nlp.morph(text)` | Таблица «слово · лемма · часть речи» по всему тексту |
| `nlp.morph_quality()` | Измеренное на корпусе качество плюс поле `explain` — разбор словами |

Последняя функция — не украшение. Разбор ошибается, и сценарий, который строит на нём
выводы, должен иметь возможность спросить, насколько ему верить, не выходя из языка.
Числа считаются на встроенном корпусе при первом обращении и дальше берутся готовыми.
