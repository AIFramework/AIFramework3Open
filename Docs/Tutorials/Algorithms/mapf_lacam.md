...
Масштабируемость и оптимальность в таблице — свойства алгоритмов из статей. Здешние реализации
экспериментальные: на обмене соседями на поле 2×2, разъезде через карман и агенте, стоящем на
чужом пути, обе возвращают решения с конфликтами, а на открытых сетках 6×6 с пятью агентами
корректны два решения из пяти. Результат обязательно проверяйте `IsValid`; для гарантированно
корректного решения берите `CBS` или `ICBS`.
