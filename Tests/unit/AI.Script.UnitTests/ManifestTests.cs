...
public sealed class ManifestTests
{
    ...
    /// </summary>
    /// <remarks>
    /// Мерится хост со всеми пространствами, включая <c>llm</c> и <c>search</c>: именно он
    /// уходит модели. Проверка на неполном хосте показывала бы запас, которого в жизни нет,
    /// и бюджет ломался бы ровно в тот момент, когда подключают последний модуль.
    /// </remarks>
    [Fact]
    public void Manifest_Index_IsSmallEnoughForAPrompt()
    {
        ...
        Assert.True(text.Length < 2000, $"индекс разросся до {text.Length} символов");
    }
