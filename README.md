...
- **NLP** — стеммер, морфология русского языка (часть речи 96 %, лемматизация 83 % на эталонном корпусе), BoW, TF-IDF, BPE/Sentence-токенизация, NER, суммаризация.
- **Имитационное моделирование** — дискретно-событийные модели, очереди, системная динамика, марковские процессы решений и агентные модели (`AI.Simulation`); прогон сверяется с аналитическими формулами теории массового обслуживания.
...
| **AI.NLP** | Стемминг, морфология и лемматизация, TF-IDF, токенизация, суммаризация |
...
| **AI.NLP** | Стеммер, морфологический разбор русского языка: определение части речи по словарю закрытых классов и суффиксам, правила склонения существительных, лемматизация с учётом части речи. Эталонный корпус из 247 разобранных вручную словоформ и измерение качества: разметка частей речи 96.2 %, лемматизация 82.8 %, каждый отчёт объясняет себя через `IInterpretable` и называет то, что правилами не берётся. Разбор доступен из языка сценариев: `nlp.pos`, `nlp.analyze`, `nlp.morph`, а `nlp.morph_quality` отдаёт измеренное качество прямо в скрипт. BoW, TF-IDF, BM25, токенизация, NER, суммаризация. |
...
| **AI.Simulation** | Дискретно-событийное моделирование с календарём событий и статистикой, теория массового обслуживания (M/M/1, M/M/c, Эрланг B и C, закон Литтла), системная динамика «запасы — потоки», марковские процессы принятия решений, каркас агентного моделирования. |
